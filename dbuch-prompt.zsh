...
prompt_dbuch_preprompt_render() {
	...
	# Set the path.
	preprompt_parts+=('%F{${prompt_dbuch_colors[path]}}%~%f')

	# Add Git branch and dirty status info.
	typeset -gA prompt_dbuch_vcs_info
	if [[ -n $prompt_dbuch_vcs_info[branch] ]]; then
		preprompt_parts+=("%F{$git_color}"'${prompt_dbuch_vcs_info[branch]}${prompt_dbuch_git_dirty}%f')
	fi
	# Git pull/push arrows.
	if [[ -n $prompt_dbuch_git_arrows ]]; then
		preprompt_parts+=('%F{$prompt_dbuch_colors[git:arrow]}${prompt_dbuch_git_arrows}%f')
	fi

	# Username and machine, if applicable.
	[[ -n $prompt_dbuch_state[username] ]] && preprompt_parts+=($prompt_dbuch_state[username])
	# Execution time.
	[[ -n $prompt_dbuch_cmd_exec_time ]] && preprompt_parts+=('%F{$prompt_dbuch_colors[execution_time]}${prompt_dbuch_cmd_exec_time}%f')
	...
	if [[ $1 == precmd ]]; then
		...
		print
	elif [[ $prompt_dbuch_last_prompt != $expanded_prompt ]]; then
		# Redraw the prompt.
...
prompt_dbuch_precmd() {
	...
	# Print the preprompt.
	prompt_dbuch_preprompt_render "precmd"
}
...
prompt_dbuch_async_refresh() {
	...
	if (( ${PURE_GIT_PULL:-1} )) && [[ $prompt_dbuch_vcs_info[top] != $HOME ]]; then
	...
	if (( time_since_last_dirty_check > ${PURE_GIT_DELAY_DIRTY_CHECK:-1800} )); then
		...
		async_job "prompt_dbuch" prompt_dbuch_async_git_dirty ${PURE_GIT_UNTRACKED_DIRTY:-1}
	fi
}
...
prompt_dbuch_async_callback() {
	...
	[[ ${prompt_dbuch_async_render_requested:-$do_render} = 1 ]] && prompt_dbuch_preprompt_render
...
prompt_dbuch_reset_prompt_symbol() {
	prompt_dbuch_state[prompt]=${PURE_PROMPT_SYMBOL:-❯}
}

prompt_dbuch_update_vim_prompt_widget() {
	setopt localoptions noshwordsplit
	prompt_dbuch_state[prompt]=${${KEYMAP/vicmd/${PURE_PROMPT_VICMD_SYMBOL:-❮}}/(main|viins)/${PURE_PROMPT_SYMBOL:-❯}}
...
prompt_dbuch_state_setup() {
	...
	[[ -n $ssh_connection ]] && username='%F{$prompt_dbuch_colors[user]}%n%f'"$hostname"

	# Show `username@host` if root, with username in default color.
	[[ $UID -eq 0 ]] && username='%F{$prompt_dbuch_colors[user:root]}%n%f'"$hostname"

	typeset -gA prompt_dbuch_state
	prompt_dbuch_state[version]="1.10.3"
	prompt_dbuch_state+=(
		username "$username"
...
prompt_dbuch_setup() {
	...
	PROMPT='%(12V.%F{$prompt_dbuch_colors[virtualenv]}%12v%f .)'

	# Prompt turns red if the previous command didn't exit with 0.
	PROMPT+='%(?.%F{$prompt_dbuch_colors[prompt:success]}.%F{$prompt_dbuch_colors[prompt:error]})${prompt_dbuch_state[prompt]}%f '

	# Indicate continuation prompt by … and use a darker color for it.
	PROMPT2='%F{242}%_… %f%(?.%F{magenta}.%F{red})${prompt_dbuch_state[prompt]}%f '

	# Store prompt expansion symbols for in-place expansion via (%). For
	# some reason it does not work without storing them in a variable first.
	typeset -ga prompt_dbuch_debug_depth
	prompt_dbuch_debug_depth=('%e' '%N' '%x')
	...
	local -A ps4_parts
	ps4_parts=(
		depth 	  '%F{yellow}${(l:${(%)prompt_dbuch_debug_depth[1]}::+:)}%f'
		compare   '${${(%)prompt_dbuch_debug_depth[2]}:#${(%)prompt_dbuch_debug_depth[3]}}'
		main      '%F{blue}${${(%)prompt_dbuch_debug_depth[3]}:t}%f%F{242}:%I%f %F{242}@%f%F{blue}%N%f%F{242}:%i%f'
